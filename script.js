...
function tabela(){
    ...
    //Criando vetor com palavras separadas.
    let palavras = txt.split(" ");
    let palavras1 = new Array(palavras.length);
    let vezes = new Array(palavras.length);

    //Criando vetor com cópia da frase.
    for(let i = 0;i<palavras.length;i++)
    {
        palavras1[i] = palavras[i];
    }

    //Criando vetor para guardar o número de vezes que cada palavra apareceu (ocorrências).
    for(let i = 0;i<palavras.length;i++)
    {
        vezes[i]=0;
    }

    //For que "limpa" palavras com vírgulas. 

    palavras = limpaTexto(palavras);


    //For para salvar o número de ocorrências de cada palavra em sua respectiva posição no vetor vezes.
    for(let i=0; i<palavras.length;i++)
    {
    ...
    //Criando lista sem palavras repetidas.
   /* let bagOfWords = [];
    for(let i=0; i<palavras.length; i++)
    {
        for(let j=0; j<=bagOfWords.length; j++)
        {
            if(bagOfWords[j] == palavras[i])
            { 
                break;
            }   
        }
    ...
    //For que cria tabela com palavras.
...
function ocorrencias()
{
    ...
    //Mostrar qual é a posição do vetor "vezes" com o maior número
...
{   
    ...
    //For que substitue a palavra especificada no vetor com o texto pela nova palavra desejada.
    ...
    //For que cria uma nova string que será exibida. 
    let Ntxt ="";
    for(let i= 0; i<txt.length; i++)
    {
        Ntxt = Ntxt + " " + txt[i];
