...
class IndexView {
    ...
    */
    ...
    createDishes(category, dishList, pedido) {
        ...
        let botao = "<button class='btn btn-primary carrinho'>Seu pedido aqui!</button>";
        
        let carrinho = "<div id='carrinho' style='display:none;'>";
        
        let divPedido = `<h3>Pedido</h3> <h5 id='totalPedido'>0</h5><ul id='pedidoList'></ul>`;
        ...
        //carregando na página
        this.domObj.innerHTML = "";
        this.domObj.innerHTML += botao;

        carrinho +=divPedido
        carrinho += "</div>";

        this.domObj.innerHTML += carrinho;
    ...
    createDish(dish) { 
        ...
        let card = `<div id=card${dish.code} class="menu-item-tile col-md-6">
        <div class="row">
            <div class="col-sm-5">
        ...
        for(let i =1; i<vetorQnt.length; i=i+2)
        {
            ...
            if(vetorQnt[i] == dish.code)
            {

                card = `<div id=card${dish.code} class="menu-item-tile col-md-6">
                <div class="row">
                    <div class="col-sm-5">
                        <div class="menu-item-photo">
                            <div class="menu-item">${dish.code}</div>
                            <img class="img-responsive" src="${dish.photo}" alt="Item" width="250" height="150">
                        </div>
                        <div>
                            <span> </span> ${dish.price} 
                            <span></span>
                    ...
                    <div style="background-color:white; width:64px; display:flex;">
                        <button id="${dish.code}" class="mais" style="color:black;">+</button><output id="qnt${dish.code}" style="margin:5px; padding:5px; color:black;">1</output><button id="${dish.code}" class="menos" style="color:black;">-</button>
                    </div>
                    </div>
                    <div class="menu-item-description col-sm-7">
                        <h3 class="menu-item-title">${dish.name}</h3>
    ...
    listPedido(pedido) {
        ...
        listaPedido.forEach((item) => {
            ...
            for(let i =1; i<vetorQnt.length; i=i+2)
            {
                if(vetorQnt[i] == item.code)
                {
                    itemsObj.innerHTML += `<li> ${vetorQnt[i+1]}x - ${item.name}<button id="${item.code}" class="btn btn-primary remove">Remover</button></li>`;
                }
            }
